...
import { Globe, Sparkles, Zap, Circle } from "lucide-react";
...
// TODO: Import Trigger.dev hook when backend is ready
...
type WorkflowStage = 'idle' | 'scraping' | 'generating' | 'testing' | 'retrying' | 'completed' | 'failed';
...
export function QueryInterface() {
	...
	// TODO: Replace with real Trigger.dev hook
	// const { submit, run, error } = useRealtimeTaskTrigger<typeof scrapeToApi>(
	//   "scrape-to-api",
	//   {
	//     accessToken: process.env.NEXT_PUBLIC_TRIGGER_PUBLIC_TOKEN!,
	//   }
	// )

	// Mock state for now (replace with run?.metadata when Trigger.dev is integrated)
	const [mockMetadata, setMockMetadata] = useState<WorkflowMetadata>({
		stage: 'idle'
	});

	const metadata = mockMetadata; // Replace with: run?.metadata as WorkflowMetadata | undefined
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		// TODO: Replace with real Trigger.dev submission
		// await submit({ url, query })

		// Mock flow for demo
		console.log('Would submit:', { url, query });
		...
		<div className="flex h-full w-full flex-col overflow-hidden">
			...
			<div className="border-b bg-background px-4 py-3 space-y-3">
				<form onSubmit={handleSubmit} className="flex gap-2">
					...
					<Button
						type="submit"
						disabled={!url.trim() || !query.trim() || stage !== 'idle'}
						className="shrink-0"
					>
						{stage === 'idle' ? (
							<>
				...
				{stage !== 'idle' && (
					<div className="flex items-center gap-2 text-sm">
						...
						<span className="text-muted-foreground">
							...
							{stage === 'generating' && '✨ v0 generating extraction code...'}
							{stage === 'testing' && `🧪 Testing code (Attempt ${metadata?.tests?.currentAttempt}/${metadata?.tests?.maxAttempts})...`}
							{stage === 'retrying' && `🔄 v0 fixing errors...`}
					...
					<div className="max-w-2xl text-center space-y-6">
						...
						<div>
							...
							<p className="text-muted-foreground">
								Enter a URL and describe what data you need - we'll generate a working API for you
							</p>
						</div>
